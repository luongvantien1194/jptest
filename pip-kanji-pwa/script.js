(function () {
  ...
  function supportsPipFromCanvas() {
    ...
    if (!c || !c.captureStream) {
      return {
        ok: false,
        reason: "Trình duyệt không hỗ trợ canvas.captureStream()."
      };
    }
    ...
    if (!v || !v.requestPictureInPicture) {
      return {
        ok: false,
        reason: "Trình duyệt không hỗ trợ video.requestPictureInPicture()."
      };
    }
  ...
  function bootFromKanjiData(arr) {
    state.items = (Array.isArray(arr) ? arr : []).map(function (raw, idx) {
      var on = String(
        raw.on_reading != null ? raw.on_reading : ""
      ).replace(/\|/g, "、");

      var kun = String(
        raw.kun_reading != null ? raw.kun_reading : ""
      ).replace(/\|/g, "、");
    ...
    if (!state.selected && state.items.length) {
      ...
      history.replaceState(
        ...
        "",
        "#kanji=" + encodeURIComponent(state.selected.id)
      );
    }
